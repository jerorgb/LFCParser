...
class Grammar:
    ...
    def compute_follow_sets(self):
        ...
        while changed:
            ...
            for nt, prods in self.productions.items():
                for prod in prods:
                    if prod[0] == 'e':  # Skip epsilon productions
                        continue
    ...
    def parse_ll1(self, input_string):
        ...
        while stack:
            ...
            # Top is a nonterminal
                ...
                if prod[0] != 'e':  # Skip epsilon
                    for symbol in reversed(prod):
                        stack.append(symbol)
            else:
                return False  # No production found
        
        return position == len(input_string)  # Should have consumed all input
    
    def check_slr1(self):
        """Check if grammar is SLR(1)"""
        
        try:
            # Check for direct left recursion (which is acceptable for SLR(1) but not for LL(1))
            ...
            # If grammar is already determined to be LL(1), and not left-recursive,
            # it should also be SLR(1), but we still need to verify
            ...
            items = []  # List of sets of items
            goto = {}  # Dictionary mapping (state_idx, symbol) to state_idx
        ...
        try:
            ...
            while True:
                ...
                if current_symbol in action[current_state]:
                    ...
                    if act == 'shift':
                        stack.append(current_symbol)  # Push symbol
                        stack.append(value)  # Push next state
                        position += 1
                    elif act == 'reduce':
                        nt, prod_idx = value
                        ...
                        if prod[0] != 'e':  # Don't pop for epsilon production
...
def parse_grammar():
    ...
    while True:
        ...
        try:
            ...
            print("Please enter a positive integer for the number of grammar production lines.")
    
    print(f"Enter {n} productions (one per line) in the format: Use A -> B C (SLR(1)) or A -> a | b (LL(1)).")
    for _ in range(n):
        line = input().strip()
        parts = line.split(' -> ')
        nt = parts[0]
...
if __name__ == "__main__":
    ...
    print("\n- :  Welcome to LFC Final proyect  : -\n")
    print("Please enter a positive integer for the number of grammar lines.\n")
    main()
